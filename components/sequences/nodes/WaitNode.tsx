...
export function WaitNode(props: NodeProps) {
  ...
  return (
    <div 
      className={`min-w-[200px] max-w-[240px] bg-white rounded-xl border-2 shadow-md transition-all duration-200 cursor-pointer ${
        isSelected 
    ...
    >
      <div className="p-4 relative">
        ...
        <div className="text-sm font-medium text-gray-700">
          {duration || (
            <span className="text-gray-400 italic text-xs">Set duration</span>
          )}
        </div>
        
        <Handle type="target" position={Position.Left} className="!bg-blue-500 !border-2 !border-white !w-3 !h-3" />
        <Handle type="source" position={Position.Right} className="!bg-blue-500 !border-2 !border-white !w-3 !h-3" />
      </div>
    </div>
  );
}
